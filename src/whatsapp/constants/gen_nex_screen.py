...
async def get_next_screen(decrypted_body: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if action == "data_exchange":
        ...
        if screen == "DETAILS":
            department_name = next(
                dept["title"] for dept in SCREEN_RESPONSES["APPOINTMENT"]["data"]["department"] if dept["id"] == data["department"]
            )
            location_name = next(
                loc["title"] for loc in SCREEN_RESPONSES["APPOINTMENT"]["data"]["location"] if loc["id"] == data["location"]
            )
            date_name = next(
                date["title"] for date in SCREEN_RESPONSES["APPOINTMENT"]["data"]["date"] if date["id"] == data["date"]
            )
    ...
    raise ValueError("Unhandled endpoint request. Make sure you handle the request action & screen logged above.")